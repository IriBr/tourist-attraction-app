...
if (!GOOGLE_API_KEY) {
  ...
  console.error('   Run: GOOGLE_PLACES_API_KEY=your_key npx ts-node src/scripts/seed-world-capitals.ts');
  process.exit(1);
}

// All continents with map display properties
const continents = [
...
// All countries with capitals and coordinates
...
const countriesByContinent: Record<string, CountryData[]> = {
  'Africa': [
    ...
    { name: 'Cameroon', code: 'CM', capital: 'Yaoundé', lat: 3.8480, lng: 11.5021, countryLat: 7.3697, countryLng: 12.3547 },
    { name: 'Central African Republic', code: 'CF', capital: 'Bangui', lat: 4.3947, lng: 18.5582, countryLat: 6.6111, countryLng: 20.9394 },
    { name: 'Chad', code: 'TD', capital: "N'Djamena", lat: 12.1348, lng: 15.0557, countryLat: 15.4542, countryLng: 18.7322 },
    ...
    { name: "Côte d'Ivoire", code: 'CI', capital: 'Yamoussoukro', lat: 6.8276, lng: -5.2893, countryLat: 7.5400, countryLng: -5.5471 },
    ...
    { name: 'São Tomé and Príncipe', code: 'ST', capital: 'São Tomé', lat: 0.3302, lng: 6.7333, countryLat: 0.1864, countryLng: 6.6131 },
    ...
    { name: 'Togo', code: 'TG', capital: 'Lomé', lat: 6.1256, lng: 1.2254, countryLat: 8.6195, countryLng: 0.8248 },
  ...
  'Asia': [
    { name: 'Afghanistan', code: 'AF', capital: 'Kabul', lat: 34.5553, lng: 69.2075, countryLat: 33.9391, countryLng: 67.7100 },
    ...
    { name: 'Georgia', code: 'GE', capital: 'Tbilisi', lat: 41.7151, lng: 44.8271, countryLat: 42.3154, countryLng: 43.3569 },
    ...
    { name: 'Maldives', code: 'MV', capital: 'Malé', lat: 4.1755, lng: 73.5093, countryLat: 3.2028, countryLng: 73.2207 },
    ...
    { name: 'Yemen', code: 'YE', capital: "Sana'a", lat: 15.3694, lng: 44.1910, countryLat: 15.5527, countryLng: 48.5164 },
  ],
  'Europe': [
    ...
    { name: 'Moldova', code: 'MD', capital: 'Chișinău', lat: 47.0105, lng: 28.8638, countryLat: 47.4116, countryLng: 28.3699 },
  ...
  'North America': [
    { name: 'Antigua and Barbuda', code: 'AG', capital: "Saint John's", lat: 17.1274, lng: -61.8468, countryLat: 17.0608, countryLng: -61.7964 },
    ...
    { name: 'Costa Rica', code: 'CR', capital: 'San José', lat: 9.9281, lng: -84.0907, countryLat: 9.7489, countryLng: -83.7534 },
    ...
    { name: 'Grenada', code: 'GD', capital: "Saint George's", lat: 12.0561, lng: -61.7488, countryLat: 12.1165, countryLng: -61.6790 },
    ...
    { name: 'United States', code: 'US', capital: 'Washington, D.C.', lat: 38.9072, lng: -77.0369, countryLat: 37.0902, countryLng: -95.7129 },
  ],
  'Oceania': [
    ...
    { name: 'Tonga', code: 'TO', capital: "Nuku'alofa", lat: -21.2114, lng: -175.1998, countryLat: -21.1790, countryLng: -175.1982 },
  ...
  'South America': [
    ...
    { name: 'Brazil', code: 'BR', capital: 'Brasília', lat: -15.8267, lng: -47.9218, countryLat: -14.2350, countryLng: -51.9253 },
    { name: 'Chile', code: 'CL', capital: 'Santiago', lat: -33.4489, lng: -70.6693, countryLat: -35.6751, countryLng: -71.5430 },
    { name: 'Colombia', code: 'CO', capital: 'Bogotá', lat: 4.7110, lng: -74.0721, countryLat: 4.5709, countryLng: -74.2973 },
    ...
    { name: 'Paraguay', code: 'PY', capital: 'Asunción', lat: -25.2637, lng: -57.5759, countryLat: -23.4425, countryLng: -58.4438 },
...
function mapGoogleTypeToCategory(types: string[]): AttractionCategory | null {
  ...
  const typeMap: Record<string, AttractionCategory> = {
    ...
    'city_hall': 'historical', 'library': 'museum', 'university': 'historical',
  };
...
async function searchAttractions(query: string, lat: number, lng: number, maxResults: number = 15): Promise<any[]> {
...
async function main() {
  ...
  console.log(' WORLD CAPITALS SEEDER');
  console.log(' Adding all countries, capitals, and attractions');
  console.log('='.repeat(70));

  let stats = { continents: 0, countries: 0, cities: 0, attractions: 0, skipped: 0 };
  ...
  for (const [continentName, countries] of Object.entries(countriesByContinent)) {
    ...
    if (!continentId || countries.length === 0) continue;
    ...
    for (const countryData of countries) {
      ...
      if (!country) {
        try {
          ...
          if (e.code === 'P2002') {
            console.log(`   ⚠️  Skipping duplicate: ${countryData.name}`);
            continue;
          }
          throw e;
        }
      ...
      // Create or get capital city
      let city = await prisma.city.findFirst({
        where: { name: countryData.capital, countryId: country.id },
      });
      ...
      if (!city) {
        ...
        stats.cities++;
        console.log(`   📍 ${countryData.capital}, ${countryData.name}`);
      }

      // Check if city already has attractions
      const attractionCount = await prisma.attraction.count({ where: { cityId: city.id } });
      if (attractionCount >= 10) {
        console.log(`   ⏭️  ${countryData.capital} already has ${attractionCount} attractions`);
        continue;
      }

      // Fetch attractions from Google Places API
      console.log(`   🔍 Fetching attractions for ${countryData.capital}...`);
      const seenPlaceIds = new Set<string>();

      for (const queryPrefix of SEARCH_QUERIES) {
        const query = `${queryPrefix} ${countryData.capital}`;
        const places = await searchAttractions(query, countryData.lat, countryData.lng);
        ...
        for (const place of places) {
          ...
          if (place.photos?.length > 0) {
            thumbnailUrl = getPhotoUrl(place.photos[0].name);
            for (let i = 0; i < Math.min(place.photos.length, 3); i++) {
              images.push(getPhotoUrl(place.photos[i].name));
            }
          }
      ...
      await delay(300); // Rate limiting between cities
    }
  }
  ...
  console.log('='.repeat(70));
  console.log(`   New Continents:   ${stats.continents}`);
  console.log(`   New Countries:    ${stats.countries}`);
  console.log(`   New Cities:       ${stats.cities}`);
  console.log(`   New Attractions:  ${stats.attractions}`);
