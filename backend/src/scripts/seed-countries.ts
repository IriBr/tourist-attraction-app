...
// All countries by continent (ISO 3166-1 alpha-2 codes)
...
async function main() {
  console.log('🌍 Seeding all countries of the world...\n');
  ...
  for (const [continentName, countries] of Object.entries(countriesByContinent)) {
    ...
    for (const country of countries) {
      try {
        ...
        // Handle duplicate country codes (some countries might have conflicts)
        if (e.code === 'P2002') {
          console.log(`  ⚠️  Skipping duplicate: ${country.name} (${country.code})`);
        } else {
          throw e;
        }
      }
    }
    console.log(`  ✓ ${continentName}: ${countries.length} countries`);
  }
